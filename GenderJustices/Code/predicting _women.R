...
# Set some parameters
user_local <- "/Users/rosamondthalken/Documents/Graduate School/Thesis/Thesis Code/Female Justices"
...
load(file.path(user_local, "Female Justices/Data/metadata.RData"))
load(file.path(user_local, "Female Justices/Data/wide_relative_df.RData"))
...
if(use_existing_data == TRUE) {
  ...
  for(i in 1:length(max_col_vals)){
    ...
    # Now remove any features that do not appear at least once in every author (fixed)
    ...
    oconnor_ids <- which(classing_data_full$Author == "OConnorCleaned")
    ginsburg_ids <- which(classing_data_full$Author == "GinsburgCleaned")
    
    small_class_size <- min(c(length(oconnor_ids), length(ginsburg_ids)))
    
    oconnor_keep <- sample(oconnor_ids, small_class_size)
    ginsburg_keep <- sample(ginsburg_ids, small_class_size)
    
    # a new data frame from the sampled data
    classing_data <- classing_data_full[c(oconnor_keep, ginsburg_keep),]
    ...
    nscFit <- train(x=training,
                    y = factor(classing_data$Author[trainIndex]),
                    method = "pam",
    ...
    train_cm <- confusionMatrix(data = training_data_class_pred, reference = factor(classing_data$Author[trainIndex]))
    ...
    test_cm <- confusionMatrix(data = class_pred, reference = factor(classing_data$Author[-trainIndex]))
    test_out[[i]] <- test_cm
  ...
  save(train_out, test_out, props_out, listgenes_out, file = file.path(user_local, "Female Justices/Data", file_name))
  save(features_used, file = file.path(user_local, "Female Justices/Data/features_used_sum.RData"))
}
...
bias_docs <- group_by(metadata, Author) %>%
  summarize(Documents = n()) %>%
  select(Bias = Author, Documents)
...
write.csv(a, "/Users/rosamondthalken/Documents/Graduate School/Thesis/Thesis Code/Female Justices/Results/a.csv")
