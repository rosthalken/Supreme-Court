...
load(file.path(user_local, "RData/metadata.RData"))
...
if(use_existing_data == TRUE) {
  if(use_punc_data){
    load(file.path(user_local, "/Users/rosamondthalken/Documents/Graduate School/Involvement/GRA/Gendered_Judges/Data/classifier_data_w_punc.RData"))
    load(file.path(user_local, "Gendered_Judges/Data/features_used_sum.RData"))
  } else {
    load(file.path(user_local, "/Users/rosamondthalken/Documents/Graduate School/Involvement/GRA/Gendered_Judges/Data/classifier_data.RData"))
    load(file.path(user_local, "Data/features_used_sum.RData"))
  }
} else {
  train_out <- list()
  test_out <- list()
...
knitr::kable(by_author, col.names = c("Justice", "Documents"), caption = "Documents by Justice with > 500 Ngrams")
...
# calculate the mean balanced accuracy for held out test data
...
a <- data.frame(1:length(max_col_vals),  max_col_vals, unlist(train_accs), unlist(test_accs))
knitr::kable(a, col.names = c("Model ID","Number of Ngrams", "Accuracy on Training Data", "Accuracy on Test Data"), caption = "Model Accuracy")
...
write_csv(x, "/Users/rosamondthalken/Documents/Graduate School/Thesis/Thesis Code/Ngram/Data/x.csv")

write_csv(strong_female_diffs, "/Users/rosamondthalken/Documents/Graduate School/Thesis/Thesis Code/Ngram/Data/strong_female_diffs.csv")

write_csv(strong_male_diffs, "/Users/rosamondthalken/Documents/Graduate School/Thesis/Thesis Code/Ngram/Data/strong_male_diffs.csv")

knitr::kable(x[1:50,], col.names = c("Ngram", "Mean Difference", "Gender"), caption = "Most Useful Ngrams")

knitr::kable(male_table[1:25, ], col.names = c("Ngram", "Mean Difference", "Gender"), caption = "Most Useful Male Ngram Features")
# wordcloud(male_table$id, freq = male_table$mean, max.words = 100, rot.per = 0, random.order = F)

knitr::kable(female_table[1:25,], col.names = c("Ngram","Mean Difference", "Gender"), caption = "Most Useful Female Ngram Features")
# wordcloud(female_table$id, freq = male_table$mean,max.words = 100, rot.per = 0, random.order = F)

# Haven't added Appendix yet