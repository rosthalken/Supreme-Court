...
for(i in 1:length(the_dirs)){
  ...
  temp_name <- paste("Dissent/RData/", the_dirs[i], ".RData", sep="")
...
long_form <- NULL
rdata_files <- dir("Dissent/RData")
for(i in 1:length(rdata_files)){
  load(file.path("Dissent/RData", rdata_files[i]))
...
save(wide_relative_df, file="Dissent/Data/wide_relative_df.RData")
rm(wide_relative_df)
