...
library(stringi)
library(tidyr)
corpus <- "Female Justices/SupremeCourtCorpusWomen"
...
for(i in 1:length(the_dirs)){
  ...
  temp_name <- paste("Female Justices/RData/", the_dirs[i], ".RData", sep="")
...
save(metadata, file="Female Justices/Data/metadata.RData")           
...
save(long_form, file="Female Justices/Data/long_form.RData")
...
save(wide_relative_df, file="Female Justices/Data/wide_relative_df.RData")
rm(wide_relative_df)
