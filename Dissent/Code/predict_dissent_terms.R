...
# some basic data about the lexicons. . . 

# male_rows <- which(meta_full$gender == "M")
# female_rows <- which(meta_full$gender == "F")
# 
# male_df <- meta_full[male_rows, 6:ncol(meta_full)]
# female_df <- meta_full[female_rows, 6:ncol(meta_full)]
# 
# male_sums <- colSums(male_df[, 6:ncol(male_df)])
# female_sums <- colSums(female_df[, 6:ncol(female_df)])

# test <- group_by(meta_full, gender) %>%
#   select(-ID, -Author, -Text_ID, -NumWords) %>%
#   summarise_all(sum)
...
if(use_existing_data == TRUE) {
  if(use_punc_data){
    load(file.path(user_local, "Data/classifier_data_w_punc.RData"))
    load(file.path(user_local, "Data/features_used_sum.RData"))
  } else {
    load(file.path(user_local, "Data/classifier_data.RData"))
    load(file.path(user_local, "Data/features_used_sum.RData"))
  }
} else {
  train_out <- list()
...
knitr::kable(a, col.names = c("Model ID","Number of Features", "Accuracy on Training Data", "Accuracy on Test Data"), caption = "Model Accuracy")
